...
namespace vaoc
{
	...
	public class AStar
	{
        ...
        public bool SearchPath(LigneCASE StartNode, LigneCASE EndNode, AstarTerrain[] tableCoutsMouvementsTerrain)
        {
        ...
        /// <returns>'true' if succeeded / 'false' if failed.</returns>
        public bool SearchPath(LigneCASE StartNode, LigneCASE EndNode, AstarTerrain[] tableCoutsMouvementsTerrain, int xmin, int xmax, int ymin, int ymax)
		{
            DateTime timeStart;
            TimeSpan perf;
        ...
        /// <param name="tableCoutsMouvementsTerrain">couts de mouvement suivant les terrains</param>
        /// <returns>'true' if succeeded / 'false' if failed.</returns>
        public bool SearchPathHPA(LigneCASE StartNode, LigneCASE EndNode, AstarTerrain[] tableCoutsMouvementsTerrain)
        {
            DateTime timeStart;
            TimeSpan perf;
        ...
        public bool NextStepHPA()
        {
            ...
            {
                LigneCASE lDebug1 = Donnees.m_donnees.TAB_CASE.FindByID_CASE(BestTrack.EndNodeHPA.ID_CASE_DEBUT);
                LigneCASE lDebug2 = Donnees.m_donnees.TAB_CASE.FindByID_CASE(BestTrack.EndNodeHPA.ID_CASE_FIN);
        ...
        private void PropagateHPA(Track TrackToPropagate)
        {
            ...
            foreach (Track A in CasesVoisinesHPA(TrackToPropagate))
            {
                ...
                {
                    LigneCASE lDebug1 = BD.Base.Case.TrouveParID_CASE(Successor.EndNodeHPA.ID_CASE_DEBUT);
                    LigneCASE lDebug2 = BD.Base.Case.TrouveParID_CASE(Successor.EndNodeHPA.ID_CASE_FIN);
		...
		{
            foreach (LigneCASE A in BD.Base.Case.CasesVoisines(TrackToPropagate.EndNode))
			{
        ...
        public List<LigneCASE> PathByNodes_O
		{
			get 
			{
				CheckSearchHasEnded();
        ...
        public List<Donnees.TAB_CASERow> PathByNodes
        {
            get
            {
                return new List<Donnees.TAB_CASERow>();
            }
        }
        ...
        private System.Collections.Generic.IList<Track> CasesVoisinesHPA(Track trackSource)
        {
            ...
            if (null != trackSource.EndNode)
            {
                ...
                if ((0 == trackSource.EndNode.I_X % m_tailleBloc) || (0 == trackSource.EndNode.I_Y % m_tailleBloc))
                {
                    ...
                    {
                        IList<LigneCASE> tableCases = BD.Base.Case.CasesVoisinesInBloc(trackSource.EndNode, m_xBlocEnd, m_yBlocEnd, m_tailleBloc);
                    ...
                    else
                    {
                        ...
                        foreach (Bloc bloc in listeBlocs)
                        {
                            ...
                            IEnumerable<LignePCC_COUTS> lignesCout = BD.Base.PccCouts.ListeLiensPointBloc(bloc.xBloc, bloc.yBloc, trackSource.EndNode.ID_CASE);
                            if (null != lignesCout && lignesCout.Count() > 0)
                            {
                                ...
                                List<LignePCC_COUTS> tableCout = BD.Base.PccCouts.CasesVoisines(lignesCout.First(), true);
                            ...
                            else
                            {
                                ...
                                IList<LigneCASE> tableCases = BD.Base.Case.CasesVoisinesInBloc(trackSource.EndNode, trackSource.blocX, trackSource.blocY, m_tailleBloc);
                                foreach (LigneCASE ligneCase in tableCases)
                                {
                                    /* debug 
                ...
                else
                {
                    ...
                    IList<LigneCASE> tableCases = BD.Base.Case.CasesVoisines(trackSource.EndNode);
                    foreach (LigneCASE ligneCase in tableCases)
                    {
                        /* debug
                        Debug.WriteLine(string.Format("CasesVoisinesHPA ajout de la case : ID={0}({1},{2}) ",
            ...
            if (null != trackSource.EndNodeHPA)
            {
                ...
                /**/
                List<LignePCC_COUTS> tableCout = BD.Base.PccCouts.CasesVoisines(trackSource.EndNodeHPA, false);
                foreach (LignePCC_COUTS ligneCout in tableCout)
                {
                    AjouterBlocHPA(trackSource, ligneCout, ref listeRetour);
                }
                ...
                LigneCASE ligneCaseFin = BD.Base.Case.TrouveParID_CASE(trackSource.EndNodeHPA.ID_CASE_FIN);
        ...
        private void AjouterBlocHPA(Track trackSource, LignePCC_COUTS ligneCout, ref System.Collections.Generic.IList<Track> listeRetour)
        {
            //LigneCASE lDebug1, lDebug2;
            Track nouveauPoint;
        ...
        static public int Cout(Track trackSource, Track trackDestination)
        {
            ...
            if (null != trackSource.EndNode && null != trackDestination.EndNodeHPA)
            {
                ...
                LigneCASE caseBloc = BD.Base.Case.TrouveParID_CASE(trackDestination.EndNodeHPA.ID_CASE_DEBUT);
            ...
            if (null != trackSource.EndNodeHPA && null != trackDestination.EndNode)
            {
                ...
                LigneCASE caseSource = BD.Base.Case.TrouveParID_CASE(trackSource.EndNodeHPA.ID_CASE_FIN);
        ...
        static public int Cout(LigneCASE caseSource, LigneCASE caseFinale)
        {
            //DateTime timeStart;
            //TimeSpan perf;
            //timeStart = DateTime.Now;
        ...
        internal bool SearchSpace(LigneCASE ligneCase, int espace, AstarTerrain[] tableCoutsMouvementsTerrain, int nombrePixelParCase, out IEnumerable<LigneCASE> listeCaseEspace, out string erreur)
        {
            DateTime    timeStart;
            TimeSpan    perf;
            //string      requete;
            bool        bEspaceFound;
            //DataSetCoutLigneCASE[] retour;
            ...
            while (!bEspaceFound)
            {
                ...
                listeCaseEspace = BD.Base.Case.ListeCaseseParCout();
                if (listeCaseEspace.Count() < m_espace)
                {
                    erreur = string.Format("SearchSpace ne trouve que {0} cases disponibles alors que l'unité en a besoin de {1}", listeCaseEspace.Count(), espace);
        ...
        {
            //if (!Initialized) throw new InvalidOperationException("You must initialize AStar before launching the algorithm.");
            if (_Open.Count == 0) return false;//c'est fini
        ...
        internal bool SearchSpace(Donnees.TAB_CASERow ligneCaseDepart, int espace, AstarTerrain[] tableCoutsMouvementsTerrain, int nombrePixelParCase, out Donnees.TAB_CASERow[] listeCaseEspace, out string erreur)
        {
            throw new NotImplementedException();
        }

        internal void SearchPathHPA(Donnees.TAB_CASERow ligneCaseDepart, Donnees.TAB_CASERow ligneCaseDestination, AstarTerrain[] tableCoutsMouvementsTerrain)
        {
            throw new NotImplementedException();
        }

        internal bool SearchPath(Donnees.TAB_CASERow ligneCaseDepart, Donnees.TAB_CASERow ligneCaseArrivee, AstarTerrain[] tableCoutsMouvementsTerrain, int xmin, int xmax, int ymin, int ymax)
        {
            throw new NotImplementedException();
        }
    }
