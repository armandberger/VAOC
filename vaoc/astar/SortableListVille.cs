...
{
	/// <summary>
    /// The SortableListVille allows to maintain a list sorted as long as needed.
	...
	/// </summary>
	[Serializable]
	public class SortableListVille : IList, ICloneable
	{
        ...
        //private Hashtable _Htable;//BEA tentative d'accélération du traitement !

		/// <summary>
		/// Default constructor.
		/// Since no IComparer is provided here, added objects must implement the IComparer interface.
		/// </summary>
		public SortableListVille() { InitProperties(null, 0); }
		...
		public SortableListVille(int Capacity) { InitProperties(null, Capacity); }

		/// <summary>
		/// Constructor.
		/// </summary>
		/// <param name="Comparer">Will be used to compare added elements for sort and search operations.</param>
		public SortableListVille(IComparer Comparer) { InitProperties(Comparer, 0); }
		...
		/// <param name="Comparer">Will be used to compare added elements for sort and search operations.</param>
		/// <param name="Capacity">Capacity of the list (<see cref="ArrayList.Capacity">ArrayList.Capacity</see>)</param>
        public SortableListVille(IComparer Comparer, int Capacity) { InitProperties(Comparer, Capacity); }
        ...
        public bool KeepSorted
        {
            set
            {
                if (value == true && !_IsSorted) throw new InvalidOperationException("The SortableListVille can only be kept sorted if it is sorted.");
                _KeepSorted = value;
            }
		...
		public object this[int Index]
		{
			...
			set
			{
                throw new InvalidOperationException("[] set method cannot be called , même que je le dit !");
        ...
        public object ValueOfCout(int Index)
        {
            return _ListCout[Index];
        }
		/// <summary>
		/// IList implementation.
        ...
        /// <exception cref="ArgumentException">The SortableListVille is set to use object's IComparable interface, and the specifed object does not implement this interface.</exception>
        ...
        public int Add(TrackVille O)
        {
            ...
            if (ObjectIsCompliant(O))
            {
                if (_KeepSorted)
                {
                    ...
                    int IndexCout = IndexOfCout(O);
                    int NewIndexCout = IndexCout >= 0 ? IndexCout : -IndexCout - 1;
                    if (NewIndexCout >= _ListCout.Count) _ListCout.Add(O);
                    else _ListCout.Insert(NewIndexCout, O);
		...
		/// Idem <see cref="ArrayList">ArrayList</see>
		/// </summary>
        public void Clear() { _List.Clear(); _ListCout.Clear(); }
        ...
        {
            throw new InvalidOperationException("Insert method cannot be called , même que je le dit !");
        ...
        public void Remove(object Value) { _List.Remove(Value); _ListCout.Remove(Value); }
        ...
        {
            TrackVille t = (TrackVille)_List[Index];
            _ListCout.Remove(t);
        ...
        public int CountCout { get { return _ListCout.Count; } }
		...
		public object Clone()
		{
            SortableListVille Clone = new SortableListVille(_Comparer, _List.Capacity);
        ...
        public int IndexOf(TrackVille O, Equality AreEqual)
		{
            int Result = -1;
            if (_IsSorted)
            {
            ...
            //return -1;
            /*
            if (_Htable.ContainsKey(O.Id))
            {
                int i = (int)_Htable[O.Id];
                if (AreEqual(_List[i], O))
                {
                    return i;
                }
            }
		...
		{
            SortableListVille SL = (SortableListVille)O;
		...
		/// <param name="C">The object to add.</param>
        /// <exception cref="ArgumentException">The SortableListVille is set to use objects's IComparable interface, and the specifed object does not implement this interface.</exception>
		/// <exception cref="ArgumentOutOfRangeException">Index is less than zero or Index is greater than Count.</exception>
		/// <exception cref="InvalidOperationException">If the object is added at the specify index, the list will not be sorted any more and the <see cref="KeepSorted"/> property is set to true.</exception>
		...
		{
            throw new InvalidOperationException("RemoveDuplicates cannot be called , même que je le dit !");
            //int PosIt;
            //if (_IsSorted)
            //{
            //    PosIt = 0;
		...
		{
            if (_UseObjectsComparison && !(O is IComparable)) throw new ArgumentException("The SortableListVille is set to use the IComparable interface of objects, and the object to add does not implement the IComparable interface.");
        ...
        private class ComparisonCase : IComparer
        {
            /// <summary>
            ...
            {
                return ((TrackVille)a).Id - ((TrackVille)b).Id;
            }
        }

        private class ComparisonCout : IComparer
        {
            /// <summary>
            /// Less than zero  a is less than b. 
            /// Zero  a equals b. 
            ...
            public int Compare(object a, object b)
            {
                return (int)(((TrackVille)a).Cost - ((TrackVille)b).Cost);
            }
        }
        ...
        private void InitProperties(IComparer Comparer, int Capacity)
		{
			...
			else
			{
                ...
                _Comparer = new ComparisonCase();
                _ComparerCout = new ComparisonCout();
                _UseObjectsComparison = true;
			}
			_List = Capacity>0 ? new ArrayList(Capacity) : new ArrayList();
            _ListCout = Capacity > 0 ? new ArrayList(Capacity) : new ArrayList();
            _IsSorted = true;
            _KeepSorted = true;
			_AddDuplicates = true;
            //_Htable = new Hashtable(Capacity);
		}
	}
}
