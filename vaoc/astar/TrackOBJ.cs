...
namespace vaoc
{
	...
	public class TrackOBJ : IComparable
	{
        ...
        public TrackOBJ(LigneCASE GraphNode)
		{
        ...
        public TrackOBJ(TrackOBJ PreviousTrack, LigneCASE caseFinale)
		{
            //if (_Target==null) throw new InvalidOperationException("You must specify a target Node for the Track class.");
			Queue = PreviousTrack;
            if (null == Queue.EndNodeHPA)
            {
                _Cost = Queue.Cost+AStarOBJ.Cout(Queue.EndNode, caseFinale);
                _OutRoad = Queue.OutRoad + AStarOBJ.HorsRoute(Queue.EndNode, caseFinale);
            }
            else
            {
                LigneCASE ligneCaseSource = BD.Base.Case.TrouveParID_CASE(Queue.EndNodeHPA.ID_CASE_FIN);                
                _Cost = Queue.Cost + AStarOBJ.Cout(ligneCaseSource, caseFinale);
                _OutRoad = Queue.OutRoad + AStarOBJ.HorsRoute(ligneCaseSource, caseFinale);
            }
			_NbArcsVisited = Queue._NbArcsVisited + 1;
			EndNode = caseFinale;
        ...
        public TrackOBJ(TrackOBJ PreviousTrack, TrackOBJ NextTrack)
        {
            //if (_Target == null) throw new InvalidOperationException("You must specify a target Node for the Track class.");
            Queue = PreviousTrack;
            //if (null == Queue.EndNodeHPA)
            ...
            //}
            _Cost = PreviousTrack.Cost + AStarOBJ.Cout(PreviousTrack, NextTrack);
            _NbArcsVisited = Queue._NbArcsVisited + 1;
