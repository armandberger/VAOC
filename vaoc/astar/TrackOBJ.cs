...
namespace vaoc
{
	...
	/// </summary>
	public class TrackOBJ : IComparable
	{
        //private LigneCASE _Target = null;
        //private static Donnees.TAB_PCC_COUTSRow _TargetHPA = null;

        //public LigneCASE Target { set { _Target = value; } get { return _Target; } }
        //public static Donnees.TAB_PCC_COUTSRow Target { set { _TargetHPA = value; } get { return _TargetHPA; } }

        private long idGlobal = 0;
        private int m_tailleBloc = 0;

        public LigneCASE EndNode;
        public LignePCC_COUTS EndNodeHPA;
        public TrackOBJ Queue;

		private int _NbArcsVisited;
		public int NbArcsVisited { get { return _NbArcsVisited; } }

        private double _Cost; // un int vaut max 2,147,483,647, ce qui veut dire qu'avec un cout de 1000 par case, on peut encore faire un trajet de 21,474 km donc si on va au-dessus, il y a un gros gros problème quelque part :-)
        ...
        public bool Succeed(LigneCASE cible) { if (null == EndNodeHPA) return (EndNode == cible); else return (EndNodeHPA.ID_CASE_FIN == cible.ID_CASE); } 

        public long Id { get; set; }
        public int blocX { get; set; }
        public int blocY { get; set; }

        public TrackOBJ(LigneCASE GraphNode, int taillebloc)
		{
        ...
        public TrackOBJ(LignePCC_COUTS GraphNode)
        {
            //if (_Target == null) throw new InvalidOperationException("You must specify a target Node for the Track class.");
            _Cost = 0;
            _OutRoad = 0;
        ...
        public TrackOBJ(TrackOBJ PreviousTrack, LigneCASE caseFinale, AStarOBJ star)
		{
            //if (_Target==null) throw new InvalidOperationException("You must specify a target Node for the Track class.");
			Queue = PreviousTrack;
            if (null == Queue.EndNodeHPA)
            {
                _Cost = Queue.Cost+star.Cout(Queue.EndNode, caseFinale);
                _OutRoad = Queue.OutRoad + star.HorsRoute(Queue.EndNode, caseFinale);
            }
            else
            {
                LigneCASE ligneCaseSource = BD.Base.Case.TrouveParID_CASE(Queue.EndNodeHPA.ID_CASE_FIN);
                _Cost = Queue.Cost + star.Cout(ligneCaseSource, caseFinale);
                _OutRoad = Queue.OutRoad + star.HorsRoute(ligneCaseSource, caseFinale);
            }
			_NbArcsVisited = Queue._NbArcsVisited + 1;
			EndNode = caseFinale;
        ...
        public TrackOBJ(TrackOBJ PreviousTrack, TrackOBJ NextTrack, AStarOBJ star)
        {
            //if (_Target == null) throw new InvalidOperationException("You must specify a target Node for the Track class.");
            Queue = PreviousTrack;
            //if (null == Queue.EndNodeHPA)
            ...
            //        _Cost = Queue.Cost + AStarOBJ.Cout(Queue.EndNode, NextTrack.EndNode);
            //    }
            //    else
            //    {
            ...
            //}
            _Cost = PreviousTrack.Cost + star.Cout(PreviousTrack, NextTrack);
            _NbArcsVisited = Queue._NbArcsVisited + 1;
		...
		{
            TrackOBJ OtherTrack = (TrackOBJ) Objet;
            return Cost.CompareTo(OtherTrack.Cost);
		}

		public static bool SameEndNode(object O1, object O2)
		{
            TrackOBJ P1 = O1 as TrackOBJ;
            TrackOBJ P2 = O2 as TrackOBJ;
            //if ( P1==null || P2==null ) throw new ArgumentException("Objects must be of 'TrackOBJ' type.");
